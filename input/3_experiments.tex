\section{Experiments}
This section provides the experimental procedures for each layer of the stack. The experiments requiring statistical accuracy comparisons (Experiment 1) use $n=5$ (42-46) independent 
SPSA seeds, extended to $n=10$ (42-51) seeds for the molecule $LiH$ so to capture the results of the bimodal convergence behavior, reported as median with a [min, max] range. Experiments
to measure the wall-clock time (Experiments 2, 4) used the fixed single seed $\texttt{seed}=42$, as on the deterministic and noiseless statevector simulation, timing has no statistical variance from seed to seed, 
rendering it unncessary to run multiple seeds. The experiment testing the IBM QPU API (Experiment 3), also reports a single run under the constraints for the 10 min/month budget of the IBM Quantum Open 
access plan. The checkpoint and stress-test experiments (5,6) were designed to validate mechanism correctness rather than statistical convergence, therefore seed choice does not apply to their pass/fail status. 
...
into exploring unphysical Hilbert space sectors (as discussed by \parencite{kandala2017hardware}).
...
\begin{itemize}
    ...
    \item \textbf{Seed}: $n=5$ (42-46) independent seeds, extended to $n=10$ (42-51) for $LiH$. Results were reported as median with [min,max] range. 
...
Both paths, the serial baseline and distributed, are constructed with guaranteed identical ansätze, Hamiltonians, paramter counts, and a minimum iterations before convergence check formula ($\max(10, \min(2n_{\text{params}}, 50))$) across all benchmark molecules
so to ensure no methodological differences arise between them. The baseline is built with the \texttt{ChemistryProblem} object, routing 
it thorugh the identical \texttt{MoleculeResolver} and ansatz construction code path, ensuring no drifts exist in a seperate re-implementation. The VQE algorithm was executed within 
a single Python process without MPI ($P=1$), providing the $T_{\text{serial}}$ value. The two paths may still 
differ due to small floating point differences in the SPSA trajectory, as the convergence criterion could trigger at a different iteration 
within the distributed path due to its per-rank Pauli term summation by \texttt{MPI_Allreduce} (which does
not sum in the same order as the single process summation in the serial path). It should also be noted that the serial baseline for $N_2$
was not completed as the projected wall-clock time on a single CPU core exceeded 10 hours, so the serial vs distributed comparison is reported with 5/6 of the 
benchmark molecules. $N_2$'s distributed wall clock time is still reported but without a serial baseline for comparison. 


% ONCE TASK D - CURRENTLY RUNNING - LANDS, THIS WILL MODIFY HEAVILY
...
$LiH$ (12 qubits, 631 Pauli terms) was selected in Strong Scaling for a fixed problem size, run at each rank 
...
random seed ($\texttt{seed}=42$) eliminates any variance between runs. For each run, the total 
...
At each rank count, a molecule is selected so that the per-rank workload (the Pauli terms/rank) scales together, as stated within Section~\ref{sec:System Architecture and Methods}.   
...
The QPU experiment was executed on March 19, 2026 against the QPU backend calibration data that was valid at the time of the remote
job submission. Results for the IBM experiment are presented in Tables~\ref{table:ibm_results} and~\ref{table:ibm_iterations}.


% WILL EXPERICIENCE DRIFTS ONCE TASK D IS COMPLETED
...
The GPU hardware used was an NVIDIA A100-SXM4-40GB (Lambda Cloud), the primary validated hardware target for this stack.  
...
of work lost on failure) against I/O overhead and uniformly applies over all benchmark molecules. 
...
evaluation on odd-numbered MPI ranks, creating asymmetrical per rank delay patterns in the system, tested through 10 VQE iterations using $H_{2}$.  
...
\subsection{Experiment 7: External Baseline Comparison (Lightining GPU, Aer-MPI)}
In Experiment 4, the contribution of the stack's GPU acceletion path vs the CPU-only results from Experiments 1 and 2 is conducted, whereas this expierment 
benchmarks the stack directly against alternate options of widely-used simulators: Pennylane Lightning-GPU and Qiskit Aer's native distributed statevector MPI mode (\texttt{AerSimulator(blocking_enable=True)}). 
This is done in contrast to this stack's per-rank replicated statevector design (\S[Acceleration Layer]), with the alternative implementing distributed statevector construction, making 
directly comparing the wall-clock time differences at the qubit count caused by the architectural variation between all options. 

All three paths (\texttt{hpchybrid}), Lightning-GPU, Aer-MPI) were all run on one instance of the A100-SXM4-40GB GPU with identical SPSA hyperparameters, seed, and ansatz configuration (HWE-adaptive, EfficientSU2, full entanglement, $\text{adaptive_reps}=\min(\text{reps}+1, 3)$)
across all 6 benchmarked molecules. The identical ansatz identity is enforced programmatically between the Lightning-GPU baseline and this stack through a comparison script asserting the Lightning parameter count 
matches the closed-form prediction $2 n_{\text{qubits}} (\text{adaptive_reps}+1)$ before being able to proceed. This ensures no comparison can silently run on different or smaller entangled circuits instead of this stack's ansatz. 

Each backend was run with a fixed iteration budget (100 iterations $H_2$, $LiH$, $BeH_2$, $H_2O$, 50 iterations for $NH_3$ and $N_2$ as their larger Pauli-term counts make 100 iterations expensive across 3 backends), rather than to convergence. % IS THIS SMART - REVIEW THIS CHOICE%  
The wall clock time is directly comparable under this budget, without the results being affected by each backend's different convergence trajectories.  

The Aer-MPI configuration uses a value \texttt{blocking_qubits} to control the statevector tile size (total chunks $= 2^{(n_{\text{qubits}} - \text{blocking_qubits})}$). 
These results use Aer's default sizing heuristic: $\text{blocking_qubits} = \max(n_{\text{qubits}}-2,\ n_{\text{qubits}}//2)$. 
A seperate sweep over $\text{blocking_qubits} \in {14, 16, 18, 19}$ for $N_2$, restricted to this range in order to avoid chunk-size blowup, 
which was observed at more aggressive tiling. Here it was found the wall-clock time varied marginally across the range (128.9-136.1 s mean of two trials per value), where no value
produced a meaningful improvement compared to the default heuristic of Aer-MPI ($\text{blocking_qubits}=18$, 129.8,s), indicating a improved performance from \texttt{hpchybrid} at $N_2$ when 
compared to Aer-MPI. 

% eported in \S[Results] to udnerstand if Aer-MPI competitiveness is sensitive to this tuning parameter
