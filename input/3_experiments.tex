...
The Aer-MPI configuration uses a value \texttt{blocking_qubits} to control the statevector tile size (total chunks $= 2^{(n_{\text{qubits}} - \text{blocking_qubits})}$). These results use Aer's default sizing heuristic: $\text{blocking_qubits} = \max(n_{\text{qubits}}-2,\ n_{\text{qubits}}//2)$. 
A seperate sweep over $\text{blocking_qubits} \in {14, 16, 18, 19}$ for $N_2$ restricted the values in order to avoid chunk-size blowup, 
which was observed at more aggressive tiling. 
